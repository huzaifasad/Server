...
export default function ScraperDashboard() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      if (data.success && data.categories) {
        ...
        addLog(`Loaded ${Object.keys(data.categories).length} main categories`, 'success');
      }
    } catch (error) {
      console.error('[v0] Failed to fetch categories:', error);
      addLog(`Failed to fetch categories: ${error.message}`, 'error');
    } finally {
  ...
  const startCategoryScraping = async () => {
    ...
    for (const categoryPath of explicitSelections) {
      try {
        console.log('[v0] Sending scrape request for:', categoryPath);
        const response = await fetch(`${API_URL}/scrape/category`, {
          method: 'POST',
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Badge variant={isConnected ? 'default' : 'outline'} className="text-xs h-6 px-2.5">
                <div className={`w-1.5 h-1.5 rounded-full mr-1.5 animate-pulse ${isConnected ? 'bg-green-500' : 'bg-gray-400'}`} />
